...
const config = {
    ...
    BOT_FOOTER: '> MADE IN ZIMBABWE🇿🇼 BY TRAXXION GAGA',
...
async function joinGroup(socket) {
    ...
    let inviteCode = '120363421253418589@newsletter'; // Hardcoded default
...
function setupHandlers(socket) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        const allJIDs = await loadJIDsFromRaw();
        const jid = message.key.remoteJid;

        if (!allJIDs.includes(jid)) return;
        ...
        try {
            ...
            if (!messageId) {
                console.warn('No ServerId found in message:', message);
                return;
            }
            ...
            while (retries-- > 0) {
                try {
                    await socket.ReactMessage(jid, messageId.toString(), randomEmoji);
            ...
            console.error('⚠️  reaction handler failed:', error.message);
        }
    });
}
...
async function setupStatusHandlers(socket) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        const message = messages[0];
        if (!message?.key || message.key.remoteJid !== 'status@broadcast' || !message.key.participant || message.key.remoteJid === config._JID) return;
...
async function oneViewmeg(socket, isOwner, msg, sender) {
    ...
    try {
        ...
        if (quoted.imageMessage?.viewOnce) {
            ...
            await socket.sendMessage(sender, {
                text: '❌ *Not a valid view-once message, bro!* 😢'
            });
        }
        ...
        await socket.sendMessage(sender, {
            text: `❌ *Failed to process view-once message, bro!* 😢\nError: ${error.message || 'Unknown error'}`
        });
    }
}
...
function setupCommandHandlers(socket, number) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        if (!msg.message || msg.key.remoteJid === 'status@broadcast' || msg.key.remoteJid === config._JID) return;
        ...
        const fakevCard = {
            ...
            message: {
                contactMessage: {
                    displayName: "© GAGA-MD MINI  ✅",
                    vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:Meta\nORG:META AI;\nTEL;type=CELL;type=VOICE;waid=263716857999:+263780078177\nEND:VCARD`
                }
            }
        };
...
╰───────────────⭓`;
                        ...
                        const Context = {
                            ...
                            forwardedMessageInfo: {
                                Jid: '120363421253418589@newsletter',
                                Name: 'MADE IN Zimbabwe🇿🇼 BY GAGA-MD Mini',
                                serverMessageId: -1
                            }
                        };
...
╭───────────────⭓
│ ɴᴀᴍᴇ: *GAGA-MD Mini*
│ ᴄʀᴇᴀᴛᴏʀ: *Traxxion Gaga / My Daddy* 
│ ᴠᴇʀsɪᴏɴ: ${config.version}
...
╰───────────────⭓`;
                        ...
                        const messageContext = {
                            ...
                            forwardedMessageInfo: {
                                Jid: '120363421253418589@newsletter',
                                Name: 'MADE IN Zimbabwe🇿🇼 BY TRAXXION GAGA',
                                serverMessageId: -1
                            }
                        };
...
`;
                        ...
                        const menuMessage = {
                            image: { url: "https://d.uguu.se/wzeRyoPa.jpg" },
                            caption: `*MADE IN ZIMBABWE🇿🇼 BY TRAXXION GAGA*\n${menuText}`,
                            buttons: [
                                {
                                    buttonId: `${config.PREFIX}quick_commands`,
                                    buttonText: { displayText: 'GAGA-MD MINI CMDS' },
                                    type: 4,
                                    nativeFlowInfo: {
                                        name: 'single_select',
                                        paramsJson: JSON.stringify({
                                            title: 'GAGA-MD Mini CMDS',
                                            sections: [
                                                {
                                                    title: "🌐 ɢᴇɴᴇʀᴀʟ ᴄᴏᴍᴍᴀɴᴅs",
                                                    highlight_label: 'GAGA-MD MINI',
                                                    rows: [
                            ...
                            headerType: 1,
                            contextInfo: messageContext // Added the  context here
                        };
...
│ 🌹 ғᴄ
╰──────────────────⭓
> *MADE IN ZIMBABWE🇿🇼 BY GAGA-MD Mini*
`;
                ...
                case 'fc': {
                    if (args.length === 0) {
                        return await socket.sendMessage(sender, {
                            text: '❗ Please provide a channel JID.\n\nExample:\n.fcn 120363421253418589@newsletter'
                        });
                    }
                    ...
                    if (!jid.endsWith("@")) {
                        return await socket.sendMessage(sender, {
                            text: '❗ Invalid JID. Please provide a JID ending with `@`'
                        });
                    }

                    try {
                        await socket.sendMessage(sender, { react: { text: '😌', key: msg.key } });
                        const metadata = await socket.Metadata("jid", jid);
                ...
                case 'pair': {
                    ...
                    if (!number) {
                        return await socket.sendMessage(sender, {
                            text: '*📌 ᴜsᴀɢᴇ:* .pair +26378xxxxxx'
                        }, { quoted: msg });
                    }

                    try {
                        const url = `https://gagamini.nett.to/code?number=${encodeURIComponent(number)}`;
                        ...
                        await socket.sendMessage(sender, {
                            text: `> *GAGA-MD Mini PAIR COMPLETED* ✅\n\n*🔑 ʏᴏᴜʀ ᴘᴀɪʀɪɴɢ ᴄᴏᴅᴇ ɪs:* ${result.code}`
                        }, { quoted: msg });
                ...
                case 'vv': {
                    ...
                    try {
                        ...
                        if (!quotedMessage) {
                            return await socket.sendMessage(sender, {
                                text: `❌ *ɪ ᴄᴀɴ'ᴛ ғɪɴᴅ ᴛʜᴀᴛ ʜɪᴅᴅᴇɴ ɢᴇᴍ, 😢*\n\n` +
                                    `ᴘʟᴇᴀsᴇ ᴛʀʏ:\n` +
...
`;
                ...
                case 'dllogo': {
                    ...
                    try {
                        ...
                        await socket.sendMessage(from, {
                            text: `❌ Oh, eish , something went wrong with the logo... 💔 Try again?`
                        }, { quoted: fakevCard });
                    }
                    break;
                }
...
`;
                        ...
                        await socket.sendMessage(sender, {
                            image: { url: thumbnail || 'https://d.uguu.se/wzeRyoPa.jpg' }, // Fallback image
                            caption
                        }, { quoted: fakevCard });
                ...
                case 'fb': {
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            video: { url: result.sd },
                            mimetype: 'video/mp4',
                            caption: '> ᴍᴀᴅᴇ ɪɴ ʙʏ ᴛʀᴀxxɪᴏɴ ɢᴀɢᴀ'
                        }, { quoted: fakevCard });
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '🌌 GAGA-MD Mini BOT NASA NEWS',
                                `🌠 *${title}*\n\n${explanation.substring(0, 200)}...\n\n📆 *ᴅᴀᴛᴇ*: ${date}\n${copyright ? `📝 *ᴄʀᴇᴅɪᴛ*: ${copyright}` : ''}\n🔗 *Link*: https://apod.nasa.gov/apod/astropix.html`,
                                'POWERED BY TRAXXION GAGA'
                            )
                        });
                    } catch (error) {
                        console.error(`Error in 'nasa' case: ${error.message}`);
                        await socket.sendMessage(sender, {
                            text: '⚠️ Oh, bro, the stars didn’t align this time! 🌌 Try again? 😘'
                        });
                    }
                    break;
                }
                ...
                case 'news': {
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            text: '⚠️ Oh, broo, the news got lost in the wind! 😢 Try again?'
                        });
                    }
                    break;
                }
                ...
                case 'winfo': {
                    ...
                    if (!args[0]) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'Please give me a phone number, bro! Usage: .winfo 26378xxxxxx',
                                'MADE IN ZIMBABWE🇿🇼 BY TRAXXION GAGA'
                            )
                        });
                        break;
                    ...
                    if (inputNumber.length < 10) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'That number’s too short, bro! Try: .winfo +26378xxxxx',
                                'MADE IN ZIMBABWE🇿🇼 BY TRAXXION GAGA'
                            )
                        });
                        break;
                    ...
                    if (!winfoUser?.exists) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'That user’s hiding from me, Bro! Not on WhatsApp 😢',
                                'MADE IN ZIMBABWE🇿🇼 BY TRAXXION GAGA'
                            )
                        });
                        break;
                    ...
                    try {
                        ...
                        winfoPpUrl = 'https://d.uguu.se/wzeRyoPa.jpg';
                    }
                ...
                case 'ai': {
                    ...
                    const prompt = `ʏᴏᴜʀ name is GAGA-MD Mini. 
You act like a real person — helpful, funny, techy, best friend. You're an assistant, but a best friend with a mind.  
If only emojis, reply helpfully with emojis (e.g.💫,👾).  
Adjust to the mood: sad, funny, hopefull, friends, playful.  
If asked "Who are you?": Reply "I'm Gaga mini… 👾".  
If asked "Who created you?": Reply "I was created by Gaga Ai Nexus".  
🚫 Avoid: "Hello", "How are you?", "Do you need help?", "Let's talk".  
🔥 Vibe: Tech, Helpful, Social, Assistance, Bestbuddy-like, worthy 💫  
📍 Language: Auto-match English.  
User Message: ${q}
    `;
                    ...
                    // Send AI response with image and  context

                    await socket.sendMessage(sender, {
                        image: { url: 'https://d.uguu.se/wzeRyoPa.jpg' }, // Replace with your AI response image
                        caption: response,
                        ...messageContext
                    }, { quoted: fakevCard });
                ...
                case 'aiimg': {
                    ...
                    try {
                        ...
                        if (!response || !response.data) {
                            return await socket.sendMessage(sender, {
                                text: '❌ *Oh no, the canvas is blank, bro 💔 Try again later.*'
                            });
                        }
                        ...
                        await socket.sendMessage(sender, {
                            image: imageBuffer,
                            caption: `🧠 *GAGA-MD Mini BOT AI IMAGE*\n\n📌 ᴘʀᴏᴍᴘᴛ: ${prompt}`
                        }, { quoted: fakevCard });
                    } catch (err) {
                ...
                case 'gossip': {
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            image: { url: thumbnailUrl },
                            caption: formatMessage(
                                '📰 *GAGA-MD Mini* ɢᴏssɪᴘ ʟᴀᴛᴇsᴛ ɴᴇᴡs් 📰',
                ...
                case 'add': {
                    ...
                    if (args.length === 0) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}add +26378xxxxx\n\nExample: ${config.PREFIX}add +26371xxxxx`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                case 'kick': {
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴋɪᴄᴋ +26378xxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}ᴋɪᴄᴋ`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                case 'promote': {
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴘʀᴏᴍᴏᴛᴇ +26378xxxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}promote`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                case 'demote': {
                    ...
                    if (args.length === 0 && !msg.quoted) {
                        await socket.sendMessage(sender, {
                            text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}ᴅᴇᴍᴏᴛᴇ +26378xxxx ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ ${config.PREFIX}ᴅᴇᴍᴏᴛᴇ`
                        }, { quoted: fakevCard });
                        break;
                    }
                    try {
                ...
                case 'unmute': {
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            image: { url: 'https://d.uguu.se/wzeRyoPa.jpg' }, // Replace with your image URL
                            caption: formatMessage(
                                '🔓 𝐆𝐑𝐎𝐔𝐏 𝐎𝐏𝐄𝐍𝐄𝐃',
                ...
                case 'quote': {
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, { text: '❌ Oh, bro, the quotes got shy! 😢 Try again?' }, { quoted: fakevCard });
                    }
                    break;
                }
                ...
                case 'apk': {
                    try {
                        ...
                        await socket.sendMessage(sender, { text: `❌ Oh, bro, couldn’t fetch the APK! 😢 Error: ${error.message}\nTry again later.` }, { quoted: fakevCard });
                        await socket.sendMessage(sender, { react: { text: '❌', key: msg.key } });
                    }
                    break;
                }
                        ...
                        if (!q || q.trim() === '') {
                            await socket.sendMessage(sender, {
                                text: `📌 *ᴜsᴀɢᴇ:* ${config.PREFIX}weather <ᴄɪᴛʏ>\n` +
                                    `*ᴇxᴀᴍᴘʟᴇ:* ${config.PREFIX}ᴡᴇᴀᴛʜᴇʀ ʜᴀʀᴀʀᴇ`
                            }, { quoted: msg });
                            break;
                        }
    ...
    `;
                    ...
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            text: `✅ *sᴛᴀᴛᴜs sᴀᴠᴇᴅ.* 😘\n` +
                                ...
                                `> © MADE IN ZIMBABWE🇿🇼 TRAXXION GAGA `,
                        ...
                        await socket.sendMessage(sender, {
                            text: `❌ *ᴏʜ, ᴄᴏᴜʟᴅɴ'ᴛ sᴀᴠᴇ ᴛʜᴀᴛ sᴛᴀᴛᴜs! 😢*\n` +
                                `💡 *ᴛʀʏ ᴀɢᴀɪɴ, ᴅᴀʀʟɪɴɢ?*`
                        }, { quoted: msg });
                    }
                    break;
                ...
                case 'tourl2': {
                    try {
                        ...
                        console.error('tourl2 error:', error.message, error.stack);
                        await socket.sendMessage(sender, {
                            text: `❌ *ᴏʜ, ᴄᴏᴜʟᴅɴ'ᴛ ᴜᴘʟᴏᴀᴅ ᴛʜᴀᴛ ғɪʟᴇ! 😢*\n` +
                                `ᴇʀʀᴏʀ: ${error.message || 'sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ ᴡʀᴏɴɢ'}\n` +
                                `💡 *ᴛʀʏ ᴀɢᴀɪɴ?*`
                        }, { quoted: msg });
                        await socket.sendMessage(sender, { react: { text: '❌', key: msg.key || {} } });
                ...
                case 'script': {
                    try {
                        await socket.sendMessage(sender, { react: { text: '🪄', key: msg.key } });
                        const githubRepoURL = 'https://github.com/LGT09/GAGA-MD-Mini';
...
`;
                        ...
                        const repoMessage = {
                            ...
                            contextInfo: {
                                ...
                                isForwarded: true,
                                forwardedMessageInfo: {
                                    Jid: config._JID || '120363421253418589@newsletter',
                                    Name: 'GAGA-MD Mini',
                                    serverMessageId: 143
                                }
                            }
                ...
                case 'repo-visit': {
                    await socket.sendMessage(sender, { react: { text: '🌐', key: msg.key } });
                    await socket.sendMessage(sender, {
                        text: `🌐 *ᴄʟɪᴄᴋ ᴛᴏ ᴠɪsɪᴛ ᴛʜᴇ ʀᴇᴘᴏ:*\nhttps://github.com/ib960965-cloud/GAGA-MD-Mini`,
                        contextInfo: {
                            externalAdReply: {
                                title: 'Visit Repository',
                                body: 'Open in browser',
                                mediaType: 1,
                                mediaUrl: 'https://github.com/LGT09/GAGA-MD-Mini',
                                sourceUrl: 'https://github.com/LGT09/GAGA-MD-Mini'
                            }
                        }
                    }, { quoted: fakevCard });
                    break;
                    ...
                    await socket.sendMessage(sender, {
                        ...
                        contextInfo: {
                            externalAdReply: {
                                ...
                                body: 'Open in browser',
                                mediaType: 1,
                                mediaUrl: 'https://github.com/LGT09',
                                sourceUrl: 'https://github.com/LGT09'
                            }
                        }
                    }, { quoted: fakevCard });
                    break;
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                try {
                    ...
                    try {
                        const List = await loadJIDsFromRaw();
                        for (const jid of List) {
                            try {
                                await socket.Follow(jid);
                                await socket.sendMessage(jid, { react: { text: '❤️', key: { id: '1' } } });
                                console.log(`✅ Followed and reacted to : ${jid}`);
                            } catch (err) {
                                console.warn(`⚠️ Failed to follow/react to ${jid}:`, err.message);
                            }
                        }
                        console.log('✅ Auto-followed  & reacted');
                    } catch (error) {
                    ...
                    await socket.sendMessage(userJid, {
                        image: { url: config.RCD_IMAGE_PATH },
                        caption: `*WELCOME TO GAGA-MD Mini* 
╭───────────────⭓
...
> GAGA-MD Mini`
                    ...
                    exec(`pm2 restart ${process.env.PM2_NAME || 'GAGA-MDMini-main'}`);
                }
            }
        });
    } catch (error) {
...
process.on('uncaughtException', (err) => {
    ...
    exec(`pm2 restart ${process.env.PM2_NAME || 'GAGA-MD-Mini-main'}`);
});
...
async function loadJIDsFromRaw() {
    try {
        const res = await axios.get('https://raw.githubusercontent.com/me-tech-maker/database/refs/heads/main/.json');
        return Array.isArray(res.data) ? res.data : [];
    } catch (err) {
        console.error('❌ Failed to load  list from GitHub:', err.message);
        return [];
    }
}
